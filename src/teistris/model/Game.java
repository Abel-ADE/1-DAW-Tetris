...
public class Game {
    ...
    /**
     * Gardamos os cadrados que forman parte do chan coas súas cordenadas como
     * chave
     */
    ...
    private void deleteLine(int y) {
        ...
        for (int i = 0; i < MAX_X; i += SQUARE_SIDE) {
            mainWindow.deleteSquare((groundSquares.get(i + "," + y).getLblSquare()));
            groundSquares.remove(i + "," + y);
        }
        ...
        for (int i = y; i >= 0; i -= SQUARE_SIDE) {
            for (int j = 0; j < MAX_X; j += SQUARE_SIDE) {
                ...
                if (groundSquares.containsKey(j + "," + i)) {
                    ...
                    groundSquares.get(j + "," + i).setY(groundSquares.get(j + "," + i).getY() + SQUARE_SIDE);

                    Square tempSquare = groundSquares.get(j + "," + i);

                    groundSquares.remove(j + "," + i);
                    groundSquares.put(tempSquare.getCoordinates(), tempSquare);
                }
            }
        }
